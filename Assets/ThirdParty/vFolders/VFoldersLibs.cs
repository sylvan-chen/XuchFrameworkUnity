        ...
        public static partial class EditorIcons
        {
            public static Texture2D GetIcon(string iconNameOrPath)
            {
                ...
                void getBuiltin()
                {
                    ...
                    if (!icon)
                        icon = EditorGUIUtility.FindTexture(iconNameOrPath);

                }
                void getFromPath()
                {
                    if (icon) return;
                    if (string.IsNullOrEmpty(iconNameOrPath)) return;
                    if (!iconNameOrPath.Contains("/") && !iconNameOrPath.Contains("\\")) return;

                    icon = AssetDatabase.LoadAssetAtPath<Texture2D>(iconNameOrPath);
                ...
                void getEmpty()
                {
                    ...
                    // Use TextAsset Icon as default fallback instead of blank texture
