...
namespace Autohand
{
    ...
    public class HandDistanceGrabber : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            primaryHand.OnBeforeGrabbed += (hand, grabbable) =>
            {
                StopPointing();
                CancelSelect();
            };
        ...
        private void OnDisable()
        {
            ...
            if (catchAssistRoutine != null)
            {
                ...
                catchAsistGrabbable.grabbable.OnGrabEvent -= (hand, grabbable) =>
                {
                    if (catchAssisted.Contains(catchAssistData))
                        catchAssisted.Remove(catchAssistData);
                };
                catchAsistGrabbable.OnPullCanceled -= (hand, grabbable) =>
                {
                    if (catchAssisted.Contains(catchAssistData))
                        catchAssisted.Remove(catchAssistData);
                };
        ...
        void CheckDistanceGrabbable()
        {
            if (useSmoothing)
            {
                ...
                currentSmoothForward = Vector3.RotateTowards(
                    currentSmoothForward,
                    forwardPointer.forward,
                    Time.deltaTime * forwardSmoothingSpeed
                    + Time.deltaTime * forwardSmoothingSpeed * currentAngleDistance,
                    1000f);
                ...
                if (didHit)
                {
                    if (hit.transform.CanGetComponent(out hitGrabbable))
                    {
                        if (targetingDistanceGrabbable == null
                            || hitGrabbable.GetInstanceID() != targetingDistanceGrabbable.GetInstanceID())
                        {
                    ...
                    else if (hit.transform.CanGetComponent(out hitGrabbableChild))
                    {
                        if (hitGrabbableChild.grabParent.transform.CanGetComponent(out hitGrabbable))
                        {
                                ...
                                || hitGrabbable.GetInstanceID() != targetingDistanceGrabbable.GetInstanceID())
                            {
                             ...
                             != targetingDistanceGrabbable.gameObject.GetInstanceID())
                    {
                        StopTargeting();
                    }
                }
                ...
                if (line != null)
                {
                    ...
                    else
                    {
                        ...
                        line.SetPositions(
                            new Vector3[]
                            {
                                forwardPointer.position, forwardPointer.position + currentSmoothForward * maxRange
                            });
                    }
                }
            }
        ...
        public virtual void StartTargeting(DistanceGrabbable target)
        {
            if (target.enabled && primaryHand.CanGrab(target.grabbable))
            {
                ...
                targetingDistanceGrabbable?.grabbable.Highlight(
                    primaryHand,
                    GetTargetedMaterial(targetingDistanceGrabbable));
        ...
        {
            targetingDistanceGrabbable?.grabbable.Unhighlight(
                primaryHand,
                GetTargetedMaterial(targetingDistanceGrabbable));
        ...
        public virtual void SelectTarget()
        {
            if (targetingDistanceGrabbable != null)
            {
                ...
                selectedEstimatedRadius = Vector3.Distance(
                    hitPoint.transform.position,
                    selectingDistanceGrabbable.grabbable.body.transform.position);
                selectingDistanceGrabbable.grabbable.Unhighlight(
                    primaryHand,
                    GetTargetedMaterial(selectingDistanceGrabbable));
                selectingDistanceGrabbable.grabbable.Highlight(
                    primaryHand,
                    GetSelectedMaterial(selectingDistanceGrabbable));
        ...
        public virtual void ActivatePull()
        {
            if (selectingDistanceGrabbable)
            {
                ...
                {
                    catchAssistRoutine = StartCoroutine(
                        StartCatchAssist(selectingDistanceGrabbable, selectedEstimatedRadius));
        ...
        void TryCatchAssist(Hand hand, Grabbable grab)
        {
            for (int i = 0; i < catchAssisted.Count; i++)
            {
                var distance = Vector3.Distance(
                                   hand.palmTransform.position + hand.palmTransform.forward * catchAssistRadius,
                                   catchAssisted[i].grab.transform.position)
                               - catchAssisted[i].estimatedRadius;
                if (distance < catchAssistRadius)
                {
                    Ray ray = new Ray(
                        hand.palmTransform.position,
                        hitPoint.transform.position - hand.palmTransform.position);
                    if (Physics.SphereCast(
                            ray,
                            0.03f,
                            out var catchHit,
                            catchAssistRadius * 2,
                            LayerMask.GetMask(Hand.grabbableLayerNameDefault, Hand.grabbingLayerName)))
                    {
        ...
        IEnumerator StartCatchAssist(DistanceGrabbable grab, float estimatedRadius)
        {
            ...
            grab.grabbable.OnGrabEvent += (hand, grabbable) =>
            {
                if (catchAssisted.Contains(catchAssistData))
                    catchAssisted.Remove(catchAssistData);
            };
            grab.OnPullCanceled += (hand, grabbable) =>
            {
                if (catchAssisted.Contains(catchAssistData))
                    catchAssisted.Remove(catchAssistData);
            };
        ...
        private void OnDrawGizmosSelected()
        {
            if (primaryHand)
                Gizmos.DrawWireSphere(
                    primaryHand.palmTransform.position
                    + primaryHand.palmTransform.forward * catchAssistRadius * 4 / 5f
                    + primaryHand.palmTransform.up * catchAssistRadius * 1 / 4f,
                    catchAssistRadius);
        }
    }
