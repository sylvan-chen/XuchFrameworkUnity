...
namespace Autohand
{
    public class HandProjector : MonoBehaviour
    {
        ...
        [ShowIf("hideHand")]
        [Tooltip(
            "The Object(s) under your main hand (not the projection hand) that contain the MeshRenderer Component(s)")]
        public Transform[] handVisuals;

        [Tooltip(
            "Should the projection interpolate between the hand pose and the projected grab pose based on the grip input axis")]
        ...
        [EnableIf("useGrabTransition")]
        [Tooltip(
            "This sets the position of the hand based on its [(gripAxis + grabTransitionOffset) * grabDistanceMultiplyer] -> gripAxis is set on the HandControllerLink component on the main hand")]
        public float grabDistanceMultiplyer = 2f;
        [Tooltip(
            "This sets the pose of the hand based on its [(gripAxis + grabTransitionOffset) * grabDistanceMultiplyer] -> gripAxis is set on the HandControllerLink component on the main hand")]
        [EnableIf("useGrabTransition")]
        ...
        void ShowProjection(bool show)
        {
            ...
            if (show)
            {
                ...
                if (targetHit.collider != null)
                {
                    ...
                    var amount = useGrabTransition
                        ? hand.grabCurve.Evaluate(
                            hand.GetTriggerAxis() * grabTransitionMultiplyer + grabTransitionOffset) : grabPercent;
                    ...
                    if (useGrabTransition
                        && (target.grabType == HandGrabType.GrabbableToHand
                            || (target.grabType == HandGrabType.Default && hand.grabType == GrabType.GrabbableToHand)))
                    {
                    ...
                    else
                    {
                        targetPos = Vector3.Lerp(
                            ...
                            handProjection.transform.position,
                            currAmount * grabDistanceMultiplyer);
                        targetRot = Quaternion.Lerp(
                            hand.transform.rotation,
                            handProjection.transform.rotation,
                            currAmount * grabDistanceMultiplyer);
                    }
                    ...
                    //Interpolate Fingers
                    postGrabPose.LerpPose(
                        ref hand.handAnimator.currentHandPose,
                        ref newProjectionPose,
                        Mathf.Clamp01(currAmount - 0.1f) * 1.25f);
                    ...
                    if (hand.GetTriggerAxis() > 0.05f || !hideHand)
                    {
                        ...
                        handProjection.transform.position = Vector3.MoveTowards(
                            lastProjectionPosition,
                            targetPos,
                            distance * Time.deltaTime * newSpeed);
                        handProjection.transform.rotation = Quaternion.RotateTowards(
                            lastProjectionRotation,
                            targetRot,
