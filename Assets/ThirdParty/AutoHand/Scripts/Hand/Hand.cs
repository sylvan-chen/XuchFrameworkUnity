...
namespace Autohand
{
    ...
    public class Hand : HandBase
    {
        ...
        [Tooltip(
            "Any layers in this mask will be removed from the spherecast checking if a grab is possible: "
        ...
        [Tooltip(
            "Whether the hand should go to the object and come back on grab, or the object to float to the hand on grab. Will default to HandToGrabbable for objects that have \"parentOnGrab\" disabled")]
        ...
        [Tooltip(
             "Makes grab smoother; also based on range and reach distance - a very near grab is minGrabTime and a max distance grab is maxGrabTime"),
         Min(0)]
        ...
        [Tooltip(
             "Increasing this value will make grabbing faster based on controller velocity during grab. Setting this value to 0 will disable this feature. When grabbing an object the speed it takes for the hand to reach the object is decreased based on the velocity of the controller while grabbing"),
         Min(0)]
        public float velocityGrabHandAmplifier = 600;

        [Tooltip(
             "Increasing this value will make grabbing faster based on grab target velocity during grab. Setting this value to 0 will disable this feature. When grabbing an object the speed it takes for the hand to reach the object is decreased based on the velocity of the controller while grabbing"),
         Min(0)]
        public float velocityGrabObjectAmplifier = 100;

        [Tooltip(
             "The point along the grab time 0-1 where the hand has been transitioned from whatever pose it was when grabbing to its open hand pose"),
         Range(0, 1)]
        public float grabOpenHandPoint = 0.5f;

        [Tooltip(
            "This is used in conjunction with custom poses. For a custom pose to work it must has the same PoseIndex as the hand. Used for when your game has multiple hands")]
...
#if UNITY_EDITOR
        private bool editorSelected = false;
#endif
        ...
        public Quaternion lastFollowRotation => handFollow.lastFrameFollowRotation;

        private List<HandTriggerAreaEvents> triggerEventAreas = new List<HandTriggerAreaEvents>();

        private float startGrabDist;
        private Vector3 startHandLocalGrabPosition;
        ...
        protected virtual void Start()
        {
            ...
            if (Selection.activeGameObject == gameObject)
            {
                ...
                Debug.Log(
                    "Auto Hand: highlighting hand component in the inspector can cause lag and quality reduction at runtime in VR. (Automatically deselecting at runtime) Remove this code at any time.",
                    this);
                editorSelected = true;
            }
            ...
            {
                if (editorSelected && Selection.activeGameObject == null)
        ...
        /// <remarks>注意，形参 targetGrabType 是无用的，它的值始终会被覆盖。这里只是为了给 override 扩展提供签名，你可以实现自己的方法用上它。</remarks>
        public virtual void Grab(GrabType targetGrabType)
        {
        ...
        public virtual void Grab(RaycastHit hit, Grabbable grab, GrabType targetGrabType = GrabType.InstantGrab)
        {
            ...
            {
                grabRoutine = StartCoroutine(GrabObject(hit, grab, targetGrabType));
            }
        }

        private GrabType GetGrabType(Grabbable grabbable)
        {
            GrabType result = this.grabType;
            if (grabbable.instantGrab)
                result = GrabType.InstantGrab;
            else if (grabbable.grabType != HandGrabType.Default)
            {
                switch (grabbable.grabType)
                {
                    case HandGrabType.HandToGrabbable:
                        result = GrabType.HandToGrabbable;
                        break;
                    case HandGrabType.GrabbableToHand:
                        result = GrabType.GrabbableToHand;
                        break;
                }
            }
        ...
        public void AutoPose(RaycastHit hit, Grabbable grabbable)
        {
            ...
            Transform poseTarget = this.palmTransform;

            if (grabbable.grabPoseType == HandGrabPoseType.Pinch)
            {
                poseTarget = pinchPointTransform;
            }

            Vector3 targetLocalPos = poseTarget.localPosition;
            Quaternion targetLocalRot = poseTarget.localRotation;
            ...
            void Calculate()
            {
                ...
                var grabDir = hit.point - poseTarget.position;
                ...
                if (Physics.ComputePenetration(
                        ...
                        hitColliderRotation,
                        palmCollider,
                        palmColliderTransform.position,
                        palmColliderTransform.rotation,
                        out var dir,
                        out var dist))
                {
                ...
                handTransform.position -= poseTarget.forward * grabDir.magnitude / 3f;
                body.position = handTransform.position;
            }
            ...
            void Align()
            {
                ...
                poseTarget.LookAt(hit.point, poseTarget.up);

                handTransform.position = palmChild.position;
                handTransform.rotation = palmChild.rotation;

                poseTarget.localPosition = targetLocalPos;
                poseTarget.localRotation = targetLocalRot;
            }
        ...
        /// <summary>Plays haptic vibration on the hand controller if supported by controller link</summary>
        public void PlayHapticVibration(float duration, float amp)
        {
            if (left)
        ...
        protected IEnumerator GrabObject(RaycastHit hit, Grabbable grab, GrabType targetGrabType)
        {
            /////////////////////////
            ////Initialize values////
            /////////////////////////
            ...
            var instantGrab = holdingObj.instantGrab || targetGrabType == GrabType.InstantGrab;
            startGrabDist = Vector3.Distance(palmTransform.position, handGrabPoint.position);
            ...
            else
            {
                ...
                foreach (var finger in fingers)
                {
                    ...
                    targetOpenPose.fingerPoses[fingerIndex].LerpDataTo(
                        ref handAnimator.closeHandPose.fingerPoses[fingerIndex],
                        finger.GetLastHitBend() / 2f);
                }
                    ...
                    for (float i = 0; i < adjustedGrabTime; i += Time.deltaTime)
                    {
                        if (holdingObj != null)
                        {
                            ...
                            if (i < adjustedGrabTime)
                            {
                                ...
                                {
                                    HandPoseData.LerpPose(
                                        ref handAnimator.handPoseDataNonAlloc,
                                        ref startGrabPose,
                                        ref targetOpenPose,
                                        grabCurve.Evaluate(point * 1f / grabOpenHandPoint));
                ...
                ////Grabbable to Hand////
                /////////////////////////
                else if (targetGrabType == GrabType.GrabbableToHand)
                {
                    holdingObj.ActivateRigidbody();
                        ...
                        if (holdingObj != null)
                        {
                            ...
                            if (point < grabOpenHandPoint)
                            {
                                HandPoseData.LerpPose(
                                    ...
                                    ref startGrabPose,
                                    ref targetOpenPose,
                                    grabCurve.Evaluate(point / grabOpenHandPoint));
                                handAnimator.handPoseDataNonAlloc.SetFingerPose(this);
                            }
                            else
                            {
                            ...
                            if (holdingObj.body != null && !holdingObj.body.isKinematic)
                            {
                                holdingObj.body.transform.position = Vector3.Lerp(
                                    startGrabbablePosition,
                                    localGrabbablePoint.position,
                                    grabCurve.Evaluate(point / grabOpenHandPoint));
                                holdingObj.body.transform.rotation = Quaternion.Lerp(
                                    startGrabbableRotation,
                                    localGrabbablePoint.rotation,
                                    grabCurve.Evaluate(point / grabOpenHandPoint));
                            ...
                            {
                                holdingObj.transform.position = Vector3.Lerp(
                                    startGrabbablePosition,
                                    localGrabbablePoint.position,
                                    grabCurve.Evaluate(point / grabOpenHandPoint));
                                holdingObj.transform.rotation = Quaternion.Lerp(
                                    startGrabbableRotation,
                                    localGrabbablePoint.rotation,
                                    grabCurve.Evaluate(point / grabOpenHandPoint));
                            }
        ...
        private bool GetClosestGrabbableHit(Grabbable grab, out RaycastHit closestHit)
        {
            closestHit = new RaycastHit { distance = float.MaxValue };
            Ray ray = new Ray { origin = palmTransform.position };
            ...
            foreach (var col in grab.grabColliders)
            {
                ...
                {
                    ray.direction = col.bounds.center - palmTransform.position;
                }
                if (col.Raycast(ray, out var hit, 1000))
                {
                ...
                {
                    ray.origin = Vector3.MoveTowards(ray.origin, col.bounds.center, 0.002f);
                    if (col.Raycast(ray, out hit, 1000) && hit.distance < closestHit.distance)
                        closestHit = hit;
                    else
                    {
                        ray.origin = Vector3.MoveTowards(ray.origin, col.bounds.center, 0.01f);
                        if (col.Raycast(ray, out hit, 1000) && hit.distance < closestHit.distance)
                            closestHit = hit;
                    }
                }
        ...
        private void OnCollisionFirstEnterEvent(GameObject collision)
        {
            OnHandCollisionStart?.Invoke(this, collision);
        }

        private void OnCollisionLastExitEvent(GameObject collision)
        {
            OnHandCollisionStop?.Invoke(this, collision);
        }

        private void OnTriggerFirstEnterEvent(GameObject collision)
        {
            OnHandTriggerStart?.Invoke(this, collision);
        }

        private void OnTriggeLastExitEvent(GameObject collision)
        {
            OnHandTriggerStop?.Invoke(this, collision);
        }
