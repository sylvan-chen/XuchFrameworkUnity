...
namespace Autohand
{
    ...
    public class CollisionTracker : MonoBehaviour
    {
        ...
        {
            get { return triggerObjects.Count; }
        }
        ...
        private void CheckTrackedObjects()
        {
            if (!disableCollisionTracking)
            {
                for (int i = 0; i < collisionObjects.Count; i++)
                {
                    if (!collisionObjects[i].activeInHierarchy || !nextCollisionObjects.Contains(collisionObjects[i]))
                    {
            ...
            if (!disableTriggersTracking)
            {
                for (int i = 0; i < triggerObjects.Count; i++)
                {
                    if (!triggerObjects[i].activeInHierarchy || !nextTriggerObjects.Contains(triggerObjects[i]))
                    {
