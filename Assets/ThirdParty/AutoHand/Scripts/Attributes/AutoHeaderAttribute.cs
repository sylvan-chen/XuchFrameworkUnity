...
namespace Autohand
{
    public class AutoHeaderAttribute : PropertyAttribute
    {
        ...
        public AutoHeaderAttribute(string label, string icon, int count = default, int depth = default)
        {
            this.count = count;
            this.depth = depth;
            this.label = label;
        ...
        public AutoHeaderAttribute(
            string label, string icon, string tooltip, string toggleName, System.Type classType, int count = default, int depth = default)
        {
            this.count = count;
            this.depth = depth;
            this.label = label;
    ...
    public class AutoHeaderDrawer : PropertyDrawer
    {
        ...
        static string currentLogoName = string.Empty;
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            var icon = string.IsNullOrEmpty(attr.icon) ? "AutohandLogo" : attr.icon;
            if (autohandlogo == null || currentLogoName != icon)
            {
                var assets = AssetDatabase.FindAssets(icon);
                if (assets.Length > 0)
                    autohandlogo = AssetDatabase.LoadAssetAtPath<Texture>(AssetDatabase.GUIDToAssetPath(assets[0]));
                currentLogoName = icon;
            }
    ...
    public static class Constants
    {
        ...
        {
            new Color(0.3411765f, 0.6039216f, 0.7803922f), new Color(0.145098f, 0.6f, 0.509804f), new Color(0.9215686f, 0.6431373f, 0.282353f),
            new Color(0.8823529f, 0.3529412f, 0.4039216f), new Color(0.9529412f, 0.9294118f, 0.682353f)
        };

        public static Color ColorForDepth(int depth) => _barColors[depth % _barColors.Length];

        public static Color BackgroundColor { get; } =
            EditorGUIUtility.isProSkin ? new Color(0.18f, 0.18f, 0.18f, 0.75f) : new Color(0.7f, 0.7f, 0.7f, 0.75f);
