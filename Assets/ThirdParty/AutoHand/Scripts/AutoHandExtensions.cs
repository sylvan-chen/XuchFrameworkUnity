...
namespace Autohand
{
    ...
    {
        private static Transform _transformRuler = null;
        ...
        private static Transform _transformRulerChild = null;
        ...
        private static Transform _transformParent = null;
       ...
       var tempComponent = componentClass.GetComponent<T>();
        ...
        public static GUIStyle LabelStyle(
            Color textColor, TextAnchor textAnchor = TextAnchor.MiddleLeft, FontStyle fontStyle = FontStyle.Normal, int fontSize = 13)
        {
            var style = new GUIStyle(GUI.skin.label);
            style.font = (Font)Resources.Load("Righteous-Regular", typeof(Font));
            style.fontSize = fontSize;
        ...
        public static GUIStyle LabelStyleB(
            Color textColor, TextAnchor textAnchor = TextAnchor.MiddleLeft, FontStyle fontStyle = FontStyle.Normal, int fontSize = 13)
        {
        ...
        public static Matrix4x4 Lerp(ref Matrix4x4 from, Matrix4x4 to, float t)
        {
            ...
            return Matrix4x4.TRS(
                ...
                Vector3.Lerp(fromScale, toScale, t));
        }
        ...
        private static readonly Collider[] touchingSphereOverlapCollidersNonAlloc = new Collider[10];

        public static bool TryGetMaxSphereRadiusForNonOverlap(
            Vector3 checkPosition, float testRadius, float minimumRadius, out float finalRadius, int layerMask)
        {
            finalRadius = testRadius;
            int attemps = 0;

            while (attemps < 100 && finalRadius > minimumRadius)
            {
                for (int i = 0; i < touchingSphereOverlapCollidersNonAlloc.Length; i++)
                {
                    touchingSphereOverlapCollidersNonAlloc[i] = null;
                }

                var bufferCount = Physics.OverlapSphereNonAlloc(
                    checkPosition,
                    finalRadius,
                    touchingSphereOverlapCollidersNonAlloc,
                    layerMask,
