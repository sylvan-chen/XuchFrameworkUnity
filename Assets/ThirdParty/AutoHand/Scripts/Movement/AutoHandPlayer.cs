...
namespace Autohand
{
    ...
    public class AutoHandPlayer : MonoBehaviour
    {
        ...
        protected static AutoHandPlayer _Instance;
        ...
        [EnableIf("useGrounding"),
         Tooltip(
             "The layers that platforming will be enabled on, will not work with layers that the HandPlayer can't collide with")]
        ...
        private IEnumerator WaitFlagForTrackingStart()
        {
        ...
        protected virtual void UpdateRigidbody()
        {
            ...
            if (pushAxis != Vector3.zero)
            {
                body.linearVelocity = Vector3.MoveTowards(
                    body.linearVelocity,
                    pushAxis,
                    pushingAcceleration * Time.fixedDeltaTime);
            ...
            if (climbAxis != Vector3.zero)
            {
                body.linearVelocity = Vector3.MoveTowards(
                    body.linearVelocity,
                    climbAxis,
                    climbingAcceleration * Time.fixedDeltaTime);
            ...
            {
                var newVel = Vector3.MoveTowards(
                    body.linearVelocity,
                    move * maxMoveSpeed,
        ...
        protected virtual void UpdateTrackedObjects()
        {
            ...
            trackingContainer.position = new Vector3(
                targetTrackedPos.x,
                trackingContainer.position.y,
                targetTrackedPos.z);

            //This slow moves the head + controllers on the Y-axis so it doesn't jump when stepping up
            if (isGrounded)
                trackingContainer.position = Vector3.MoveTowards(
                    trackingContainer.position,
                    targetTrackedPos + Vector3.up * heightOffset,
                    (Mathf.Abs(trackingContainer.position.y - targetTrackedPos.y) + 0.1f)
                    * Time.deltaTime
                    * heightSmoothSpeed);
            else
            ...
            targetPos.y = 0;
            targetPosOffset = Vector3.MoveTowards(
                targetPosOffset,
                targetPos,
                body.linearVelocity.magnitude * Time.deltaTime);
        ...
        private void PreventHandClipping(Hand hand, Vector3 startPosition)
        {
        ...
        protected virtual void SyncBodyHead()
        {
                        ...
                        while (overlapCount > 0 && attempts < bodySyncMaxIterations)
                        {
                            ...
                            for (int j = 0; j < overlapCount; j++)
                            {
                                ...
                                if ((handLeft.IsHolding() && handLeft.holdingObj.grabColliders.Contains(otherCollider))
                                    || (handRight.IsHolding()
                                        && handRight.holdingObj.grabColliders.Contains(otherCollider)))
                                    continue;
                                ...
                                if (Physics.ComputePenetration(
                                        ...
                                        out var closestDepenetrationDistance))
                                {
                                    ...
                                    averageDepentration += closestDepentrationDirection
                                                           * (closestDepenetrationDistance * 1.05f);
                                    averageDepentration.y = 0;
                                }
            ...
            void GetCapsuleEndPoints(CapsuleCollider col, out Vector3 top, out Vector3 bottom, out float radius)
            {
                Transform colTransform = col.transform;
                Vector3 capsuleCenter = colTransform.TransformPoint(col.center);
                float actualHeight = col.height * 0.5f - col.radius;
                float scaledHeight = actualHeight * Vector3.Scale(colTransform.lossyScale, Vector3.up).magnitude;
                radius = col.radius
                         * Mathf.Max(colTransform.lossyScale.x, colTransform.lossyScale.y, colTransform.lossyScale.z);
                ...
                while (overlapCount > 0 && attempts < headCollisionMaxIterations)
                {
                    ...
                    for (int j = 0; j < overlapCount; j++)
                    {
                        ...
                        if (Physics.ComputePenetration(
                                ...
                                otherCollider.transform.position,
                                otherCollider.transform.rotation,
                                headPhysicsFollower.headCollider,
                                cameraHeadPosition,
                                transform.rotation,
                                out var closestDepentrationDirection,
                                out var closestDepenetrationDistance))
                    ...
                    cameraHeadPosition = Vector3.MoveTowards(
                        cameraHeadPosition,
                        currentHeadPosition,
                        headRadius * 0.95f);
        ...
        protected virtual void UpdateTurn(float deltaTime)
        {
            ...
            {
                lastUpdatePosition = new Vector3(transform.position.x, lastUpdatePosition.y, transform.position.z);
                trackingContainer.RotateAround(
                    transform.position,
                    Vector3.up,
                    smoothTurnSpeed * (Mathf.MoveTowards(turningAxis, 0, turnDeadzone)) * deltaTime);
        ...
        private RaycastHit[] hitsNonAlloc = new RaycastHit[128];
        ...
        protected virtual void Ground()
        {
            ...
            if (!tempDisableGrounding && useGrounding && !IsClimbing() && !(pushAxis.y > 0))
            {
                ...
                float stepAngle;
                float dist;
                float scale = transform.lossyScale.x > transform.lossyScale.z ? transform.lossyScale.x
                    : transform.lossyScale.z;

                var stepHeight = this.maxStepHeight;
                stepHeight *= climbAxis.y > 0 ? climbUpStepHeightMultiplier : 1;
                stepHeight *= pushAxis.y > 0 ? pushUpStepHeightMultiplier : 1;
                stepHeight *= scale;

                var point1 = scale * bodyCapsule.center
                             + transform.position
                             + scale * bodyCapsule.height / 2f * -Vector3.up
                             + (stepHeight + scale * bodyCapsule.radius * 2) * Vector3.up;
                var point2 = scale * bodyCapsule.center
                             + transform.position
                             + (scale * bodyCapsule.height / 2f + groundingPenetrationOffset) * -Vector3.up;
                ...
                int hitCount = Physics.SphereCastNonAlloc(
                    ...
                    -Vector3.up,
                    hitsNonAlloc,
                    Vector3.Distance(point1, point2) + scale * bodyCapsule.radius * 4,
                    groundLayerMask,
                    QueryTriggerInteraction.Ignore);
                ...
                void CheckGroundHits()
                {
                    for (int i = 0; i < hitCount; i++)
                    {
                        ...
                        {
                            if (hit.point.y >= point2.y
                                && hit.point.y <= point2.y + stepHeight + groundingPenetrationOffset)
                            {
        ...
        protected virtual void UpdatePlayerHeight()
        {
            ...
            {
                playerHeight = Mathf.Clamp(
                    headCamera.transform.position.y - transform.position.y,
                    minMaxHeight.x,
                    minMaxHeight.y);
        ...
        {
            if (isGrounded
                && lastGroundHit.transform != null
                && (platformingLayerMask == (platformingLayerMask | (1 << lastGroundHit.collider.gameObject.layer))))
            {
                ...
                {
                    if (lastPlatformingHit.transform.position != lastPlatformPosition
                        || lastPlatformingHit.transform.rotation.eulerAngles != lastPlatformRotation.eulerAngles)
                    {
                        lastPlatformingHit = lastGroundHit;
                        ...
                        var deltaRot =
                            (lastPlatformingHit.transform.rotation * Quaternion.Inverse(lastPlatformRotation));

                        ruler.transform.RotateAround(
                            lastPlatformingHit.transform.position,
                            Vector3.up,
                            deltaRot.eulerAngles.y);
                        trackingContainer.RotateAround(
                    ...
                    if (push.Key.enabled && !push.Value.IsGrabbing())
                    {
                        ...
                        if (distance > 0)
                            offset = Vector3.Scale(
                                (push.Value.body.position - push.Value.moveTo.position),
        ...
        protected virtual void StartClimb(Hand hand, Grabbable grab)
        {
            ...
            if (!climbing.ContainsKey(hand)
                && grab != null
                && grab.CanGetComponent(out Climbable climbbable)
                && climbbable.enabled)
            {
                if (climbing.Count == 0)
                {
