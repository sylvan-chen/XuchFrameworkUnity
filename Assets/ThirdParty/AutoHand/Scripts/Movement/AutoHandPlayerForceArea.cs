...
namespace Autohand
{
    public class AutoHandPlayerForceArea : MonoBehaviour
    {
        ...
        private readonly Collider[] colliders = new Collider[30];
        ...
        private void FixedUpdate()
        {
            ...
            var radius = Enumerable.Range(0, 3).Select(xyz => xyz == player.bodyCollider.direction ? 0 : r[xyz]).Select(Mathf.Abs).Max();
            ...
            for (int i = 0; i < overlaps; i++)
            {
                ...
                {
                    if (colliders[i].attachedRigidbody != player.body
                        && colliders[i].attachedRigidbody != player.handRight.body
                        && colliders[i].attachedRigidbody != player.handLeft.body)
                    {
