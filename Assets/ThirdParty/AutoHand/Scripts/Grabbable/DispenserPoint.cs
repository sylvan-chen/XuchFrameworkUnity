...
namespace Autohand
{
    ...
    public class DispenserPoint : MonoBehaviour
    {
        ...
        [Tooltip(
            "If true the object will not just reset its position on reset it will be destroyed and a new copy will be placed. Less performant but important for things like ammo that should always respawn as new clips full")]
        ...
        {
            if (maxDistance > 0
                && currentDispense.gameObject.activeInHierarchy
                && Vector3.Distance(transform.position, currentDispense.rootTransform.position) > maxDistance)
                Dispense();
        }
        ...
        public virtual Grabbable Dispense()
        {
            if (dispenseRoutine == null)
            {
                ...
                if (poolCount < maxCopies
                    || dispensePool[poolIndex] == null
                    || dispensePool[poolIndex].activeInHierarchy == false)
