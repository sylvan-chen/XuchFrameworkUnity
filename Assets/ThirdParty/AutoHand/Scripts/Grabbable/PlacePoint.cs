...
namespace Autohand
{
    ...
    public class PlacePoint : MonoBehaviour, IGrabbableEvents
    {
        ...
        [Tooltip("The radius of the place point (relative to scale)"), ShowIf(nameof(IsShapTypeSphere))]
        public float placeRadius = 0.1f;
        [Tooltip("The radius of the place point (relative to scale)"), ShowIf(nameof(IsShapTypeBox))]
        ...
        [Tooltip("The local offset of the enter radius of the place point (not the offset of the placement)"),
         FormerlySerializedAs("radiusOffset")]
        public Vector3 shapeOffset;

        [Space]
        [Tooltip(
            "This will make the place point itself targetable for grab istead of just the object inside. Functionally just makes the place point an easier grab target, also essential if turning off the colliders on the placed object")]
        ...
        [Tooltip(
            "If true and will force hand to release on place when force place is called. If false the hand will attempt to keep the connection to the held object (but can still break due to max distances/break forces)")]
        [ShowIf("forcePlace")]
        ...
        [Tooltip(
            "This will resize the object, depending on it's render bounds, to fit into the place point radius (- resizeOffset) on place")]
        [Space]
        ...
        [Space]
        [Tooltip(
            "Whether or not the placed object should have its rigidbody disabled on place, good for parenting placed objects under dynamic objects")]
        ...
        [Tooltip(
            "Whether or not this place point should be disabled on placement. It will maintain its connection and can no longer accept new items. Causes less overhead if true")]
        public bool disablePlacePointOnPlace = false;
        [Tooltip(
            "Whether or not the placed object should be disabled on placement (this will hide the placed object and leave the place point active for a new object)")]
        ...
        [Tooltip(
            "Will allow placement for any grabbable with a name containing this array of strings, leave blank for any grabbable allowed")]
        ...
        protected virtual void Awake()
        {
            ...
            if (grabbablePlacePoint)
            {
                ...
                if (shapeType == PlacePointShape.Sphere
                    && !gameObject.CanGetComponent<SphereCollider>(out var sphereCollider))
                {
        ...
        protected virtual void CheckInvalidSettings()
        {
            ...
            {
                Debug.LogWarning(
                    "Place Points placed under a grabbable cannot support parenting other rigidbody grabbables, disable rigibody on place is being enabled",
                    this);
        ...
        public virtual void Place(Grabbable placeObj)
        {
            ...
            if (resizeOnPlace)
            {
                ...
                var scale = Mathf.Abs(
                    transform.lossyScale.x < transform.lossyScale.y ? transform.lossyScale.x : transform.lossyScale.y);
                ...
                if (shapeType == PlacePointShape.Sphere)
                    ...
                    FitAndCenterToBounds(
                        placeObj.rootTransform.gameObject,
                        (placeSize + placeSize * resizeOffset) * scale);
            }
            ...
            if (placedObject == null && highlightingObj == null)
            {
                ...
                switch (shapeType)
                {
                    case PlacePointShape.Sphere:
                        overlaps = Physics.OverlapSphereNonAlloc(
                            overlapCenterPos,
                            placeRadius * scale,
                            collidersNonAlloc,
                            placeLayers);
                    ...
                    case PlacePointShape.Box:
                        overlaps = Physics.OverlapBoxNonAlloc(
                            overlapCenterPos,
                            placeSize / 2f * scale,
                            collidersNonAlloc,
                            transform.rotation,
                            ...
                            if (CanPlace(tempGrabbable))
                            {
                                ...
                                if (existingPlacePoint)
                                {
                                    ...
                                    var concurrentCenterPos = existingPlacePoint.placedOffset.position
                                                              ...
                                                              * existingPlacePoint.shapeOffset;
        ...
        private bool IsShapTypeSphere => shapeType == PlacePointShape.Sphere;
        private bool IsShapTypeBox => shapeType == PlacePointShape.Box;
    }
}