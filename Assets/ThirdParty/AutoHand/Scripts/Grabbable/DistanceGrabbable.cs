...
namespace Autohand
{
    ...
    [HelpURL(
        "https://app.gitbook.com/s/5zKO0EvOjzUDeT2aiFk3/auto-hand/grabbable/distance-grabbinghttps://app.gitbook.com/s/5zKO0EvOjzUDeT2aiFk3/auto-hand/grabbable/distance-grabbing")]
    public class DistanceGrabbable : MonoBehaviour
    {
        ...
        private bool IsGrabTypeVelocity => grabType == DistanceGrabType.Velocity;

        [Range(0.4f, 1.1f)]
        [Tooltip(
            "Use this to adjust the angle of the arch that the gameobject follows while shooting towards your hand.")]
        [ShowIf("IsGrabTypeVelocity")]
        ...
        [Header("Rotation")]
        [Tooltip(
            "This enables rotation which makes the gameobject orient to the rotation of you hand as it moves through the air. All below rotation variables have no use when this is false.")]
        [ShowIf("IsGrabTypeVelocity")]
        ...
        private void GravitateTowardsHand()
        {
            if (gravitationEnabled)
            {
                ...
                lastGravitationVelocity = (target.position - transform.position).normalized
                                          * Time.fixedDeltaTime
                                          * gravitationVelocity;
        ...
        private void InitialVelocityPushToHand()
        {
            ...
            {
                if (archMultiplier > 0)
                    calculatedNecessaryVelocity = CalculateTrajectoryVelocity(
                        transform.position,
