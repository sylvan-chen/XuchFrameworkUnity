...
namespace Autohand
{
    ...
    public class Grabbable : GrabbableBase, IGrabbableEvents
    {
        ...
        [Tooltip(
            "The required force to break the fixedJoint\n "
        ...
        [Min(0),
         Tooltip(
             "I.E. Grab Prioirty - BIGGER IS BETTER - divides highlight distance by this when calculating which object to grab. Hands always grab closest object to palm")]
        public float grabPriorityWeight = 1;

        [Tooltip(
             "The number of seconds that the hand collision should ignore the released object\n (Good for increased placement precision and resolves clipping errors)"),
         Min(0)]
        ...
        [Tooltip(
            "For the special use case of having things connected to the grabbable that the hand should ignore while being held (good for doors and drawers) -> for always active use the [GrabbableIgnoreHands] Component")]
        public List<Collider> heldIgnoreColliders = new List<Collider>();

        [Space]
        [Tooltip(
             "Whether or not the break call made only when holding with multiple hands - if this is false the break event can be called by forcing an object into a static collider"),
         HideInInspector]
...
#if UNITY_EDITOR
        private void EditorCopyGrabbable()
        {
        ...
        public void Start()
        {
            ...
            if (Selection.activeGameObject == gameObject)
            {
                ...
                Debug.Log(
                    "Auto Hand: highlighting grabbables and rigidbodies in the inspector can cause lag and quality reduction at runtime in VR. (Automatically deselecting at runtime) Remove this code at any time.",
                    this);
                editorSelected = true;
            }
        ...
        public override void Awake()
        {
            ...
            if (body.transform != transform && !body.gameObject.HasGrabbable(out Grabbable _))
            {
            ...
            void ConnectGrabbablesRecursive(Transform obj)
            {
                ...
                {
                    if (obj.CanGetComponent<Grabbable>(out var grab))
                    {
                        if (!grabbableChildren.Contains(grab))
                            grabbableChildren.Add(grab);

                        if (!grab.grabbableParents.Contains(this))
                            grab.grabbableParents.Add(this);
                    }
        ...
        public void IgnoreColliders(Collider col, bool ignore = true)
        {
            foreach (var col2 in grabColliders)
                Physics.IgnoreCollision(col, col2, ignore);
        }
        ...
        public void IgnoreColliders(Collider[] colliders, bool ignore = true)
        {
            foreach (var col1 in grabColliders)
                foreach (var col2 in colliders)
                    Physics.IgnoreCollision(col2, col1, ignore);
        }
        ...
        private void TryCreateHighlight(Material customMat, Hand hand)
        {
            ...
            if (highlightMat != null && !highlightObjs.ContainsKey(highlightMat))
            {
                ...
                bool AddHighlightObject(Transform obj)
                {
                    ...
                    if (obj.CanGetComponent(out MeshRenderer meshRenderer))
                    {
                        //Creates a slightly larger copy of the mesh and sets its material to highlight material
                        var highlightGo = new GameObject();
                        highlightGo.transform.parent = obj;
                        highlightGo.transform.localPosition = Vector3.zero;
                        highlightGo.transform.localRotation = Quaternion.identity;
                        highlightGo.transform.localScale = Vector3.one * 1.001f;
                        highlightGo.AddComponent<MeshFilter>().sharedMesh = obj.GetComponent<MeshFilter>().sharedMesh;
                        ...
                        highlightObjs[highlightMat].Add(highlightGo);
                    }
        ...
        {
            foreach (var col in heldIgnoreColliders)
                hand.HandIgnoreCollider(col, true);
        ...
        protected internal virtual void BreakHandConnection(Hand hand)
        {
            ...
            foreach (var col in heldIgnoreColliders)
                hand.HandIgnoreCollider(col, false);
        ...
        public void AddJointedBody(Rigidbody jointedBody)
        {
            if (!jointedBodies.Contains(jointedBody))
            {
                if (jointedBody.gameObject.HasGrabbable(out Grabbable otherGrabbable))
                {
                    ...
                    jointedParents.Add(jointedBody.transform.parent);

                jointedBodies.Add(jointedBody);
            }
        }

        /// <summary>Remove a jointed rigidbody in the jointedBodies list</summary>
        public void RemoveJointedBody(Rigidbody jointedBody)
        {
            if (jointedBodies.Contains(jointedBody))
            {
                var i = jointedBodies.IndexOf(jointedBody);

                jointedBodies.RemoveAt(i);

                if (jointedBody.gameObject.HasGrabbable(out var otherGrabbable))
                {
                    if (jointedGrabbables.Contains(otherGrabbable))
                    {
                    ...
                    jointedBody.transform.parent = jointedParents[i];
        ...
        private void MakeChildrenGrabbable()
        {
            ...
            void AddChildGrabbableRecursive(Transform obj)
            {
                if (obj.CanGetComponent(out Collider col)
                    && col.isTrigger == false
                    && !obj.CanGetComponent<IGrabbableEvents>(out _)
                    && !obj.CanGetComponent<GrabbableChild>(out _)
                    && !obj.CanGetComponent<PlacePoint>(out _))
                {
        ...
        private bool rigidbodyGrabbedState;
        ...
        protected internal void ResetGrabbableAfterRlease()
        {
            if (!beingDestroyed)
            {
                if (HeldCount() == 0
                    ...
                    && parentOnGrab
                    && (placePoint == null || !(placePoint.placedObject == this && placePoint.parentOnPlace)))
                {
        ...
        public bool IsHolding(Rigidbody targetBody)
        {
            return heldBy.Any(holding => holding.body == targetBody);
        }

        public bool IsHolding(Hand hand)
        {
            return heldBy.Any(held => held == hand);
        }
