...
namespace Autohand
{
    ...
    public class GrabbableBase : MonoBehaviour
    {
        ...
        {
            get => _placePoint;
            protected set => _placePoint = value;
        }

        protected List<PlacePoint> _childPlacePoints = new List<PlacePoint>();

        public List<PlacePoint> childPlacePoints => _childPlacePoints;

        internal List<Collider> _grabColliders = new List<Collider>();

        public List<Collider> grabColliders => _grabColliders;
        ...
        public List<Hand> heldBy => _heldBy;

        private List<Hand> _beingGrabbedBy = new List<Hand>();

        public List<Hand> beingGrabbedBy => _beingGrabbedBy;
        ...
        public Transform rootTransform
        {
            get
            {
                ...
                else if (gameObject.CanGetComponent<Rigidbody>(out var rb))
                    return rb.transform;
        ...
        public virtual void Awake()
        {
            ...
            if (Selection.activeGameObject == gameObject)
            {
                ...
                Debug.Log(
                    "Auto Hand (EDITOR ONLY): Selecting the grabbable in the inspector can cause lag and quality reduction at runtime. (Automatically deselecting at runtime) Remove this code at any time.",
                    this);
                editorSelected = true;
            }
        ...
        {
            foreach (var transToIdx in originalLayers)
            {
                transToIdx.Key.gameObject.layer = newLayer;
            }
        }
        ...
        protected internal void ResetOriginalLayers()
        {
            foreach (var transToIdx in originalLayers)
            {
                transToIdx.Key.gameObject.layer = transToIdx.Value;
            }
        }

        private Dictionary<Grabbable, Coroutine> ignoreGrabbableCollisions = new Dictionary<Grabbable, Coroutine>();
                ...
                foreach (var col2 in other.grabColliders)
                {
                    if (col1.enabled
                        ...
                        && Physics.ComputePenetration(
                            col1,
                            col1.transform.position,
                            col1.transform.rotation,
                            col2,
                            col2.transform.position,
        ...
        private Dictionary<Hand, Coroutine> ignoreHandCollisions = new Dictionary<Hand, Coroutine>();
