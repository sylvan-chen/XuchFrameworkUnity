...
namespace Autohand
{
    public class HandGrabbableHighlighter : MonoBehaviour
    {
        ...
        public Hand hand
        {
            get
            {
                if (_hand == null) _hand = GetComponent<Hand>();
                return _hand;
            }
        }

        [Tooltip(
            "When choosing between multiple targets to highlight, "
        ...
        [Tooltip(
            "Whether the highlighter should ignore or check for grabbabels that are using trigger colliders or just solid colliders - reocommend using collide for most cases."
        )]
        ...
        IEnumerator HighlightUpdate(float timestep)
        {
            ...
            if (hand.left) yield return new WaitForSecondsRealtime(timestep / 2);
        ...
        {
            if (hand.holdingObj != null || hand.IsGrabbing()) currentHighlightTarget = null;
        }
        ...
        {
            if ((overrideIgnoreHighlight || hand.usingHighlight)
             && hand.highlightLayers != 0
             && (overrideIgnoreHighlight || hand.holdingObj == null && !hand.IsGrabbing()))
            {
                int grabbingLayer = LayerMask.NameToLayer(Hand.grabbingLayerName);
                int gabbingMask = LayerMask.GetMask(Hand.grabbingLayerName);
                highlightColliderNonAllocCount = Physics.OverlapSphereNonAlloc(
                    hand.palmTransform.position + hand.palmTransform.forward * hand.reachDistance / 3f,
                    hand.reachDistance,
                    highlightCollidersNonAlloc,
                    hand.highlightLayers & ~(hand.ignoreGrabCheckLayers.value),
                    highlightQuery
                );
                ...
                {
                    Vector3 dir = HandClosestHit(
                        out _highlightHit,
                        out IGrabbableEvents newLookingAtObjEvent,
                        ~(hand.handLayers | hand.ignoreGrabCheckLayers.value)
                    );
                    Grabbable newLookingAtObj = null;
                    if (newLookingAtObjEvent != null
                     && newLookingAtObjEvent.GetGrabbable() != null
                     && newLookingAtObjEvent.GetGrabbable().enabled == true)
                    ...
                    if (dir != Vector3.zero && (newLookingAtObj != null && newLookingAtObj.CanGrab(hand)))
                    {
                        ...
                        if (newLookingAtObj != currentHighlightTarget)
                        {
                            ...
                            if (currentHighlightTarget != null)
                            {
                                if (!ignoreHighlightEvents && OnStopHighlight != null) OnStopHighlight.Invoke(hand, currentHighlightTarget);
                            ...
                            if (!ignoreHighlightEvents && OnHighlight != null) OnHighlight.Invoke(hand, currentHighlightTarget);
        ...
        public virtual Vector3 HandClosestHit(out RaycastHit closestHit, out IGrabbableEvents grabbable, int layerMask, Grabbable target = null)
        {
            ...
            int overlapCount = Physics.OverlapSphereNonAlloc(
                palmPosition + palmForward * (checkSphereRadius * 0.5f),
                checkSphereRadius,
                handHighlightNonAlloc,
                layerMask,
                highlightQuery
            );
            ...
            for (int i = 0; i < overlapCount; i++)
            {
                ...
                if (ray.direction != Vector3.zero
                 && Vector3.Angle(ray.direction, hand.palmTransform.forward) < 120
                 && Physics.Raycast(ray, out hit, checkSphereRadius * 2, layerMask, highlightQuery))
                {
                    rayHitObject = hit.collider.gameObject;
                    if (closestGrabs.Count > 0) lastRayHitGrabbable = closestGrabs[closestGrabs.Count - 1];
            ...
            if (closestHitCount > 0)
            {
                ...
                {
                    if (hand.handGrabPoint.parent != closestHit.transform) hand.handGrabPoint.parent = closestHit.collider.transform;
