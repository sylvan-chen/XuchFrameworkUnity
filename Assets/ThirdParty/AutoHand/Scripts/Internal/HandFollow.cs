...
namespace Autohand
{
    ...
    public class HandFollow : MonoBehaviour
    {
        ...
        [Tooltip(
            "The drag multiplier the hand will lerp between the (start drag), and the (start drag * this) to when less than the dragDamperDistance from the follow target")]
        ...
        [Tooltip(
            "The angular drag multiplier the hand will lerp between the (start angular drag), and the (start angular drag * this) to when less than the angleDragDamperDistance from the follow target")]
        ...
        [Tooltip(
            "If the hand is holding an unparented grabbable (see gabbable.parentOnGrab) and hits its max distance it will drop the object if it cannot returm within the max distance after this many frames - this is a safety to prevent the hand from getting stuck of grabbing something it isnt supposed to")]
        public int maxDistanceNoParentReleaseFrames = 1;

        [Tooltip(
            "If the hand is holding a parented grabbable (see gabbable.parentOnGrab) and hits its max distance it will drop the object if it cannot returm within the max distance after this many frames - this is a safety to prevent the hand from getting stuck of grabbing something it isnt supposed to")]
        ...
        public Vector3 lastFollowDeltaPosition => follow.position - lastFrameFollowPosition;

        public Quaternion lastFollowDeltaRotation => follow.rotation * Quaternion.Inverse(lastFrameFollowRotation);

#if UNITY_EDITOR
        [FoldoutGroup("Debug")]
        public bool showMoveToPoint = false;
#endif
        ...
        private Vector3 lastSetMoveToFollowPosition;
        private Quaternion lastSetMoveToFollowRotation;
        private float lastSetMoveToTime;
        private float lastSetVelocityTime;
        private float lastSetAngularVelocityTime;
        private float lastSetAverageMoveToTime;
        private float lastSetMassTime;

        private float currentFollowRotationStrength;
        private float currentFollowPositionStrength;

        private Transform _moveTo = null;

#if UNITY_EDITOR
        public GameObject moveToPoint { get; private set; }
#endif
        ...
        public Transform moveTo
        {
            get
            {
                ...
                if (_moveTo == null)
                {
                    ...
                    if (showMoveToPoint)
                    {
                        var moveToPointPrefab = Resources.Load<GameObject>("MoveToPoint");
                        moveToPoint = Instantiate(moveToPointPrefab, _moveTo, false);
                        moveToPoint.transform.localPosition = Vector3.zero;
                        moveToPoint.transform.localRotation = Quaternion.identity;
                    }
#endif
                }
        ...
        protected virtual void UpdateHandPhysicsMovement()
        {
            ...
            if (follow != null && hand.enableMovement && !hand.forwardFollow)
            {
        ...
        protected virtual void UpdateHandOffset()
        {
            ...
            if (hand.enableMovement && !hand.forwardFollow)
            {
                ...
                if (hand.holdingObj && !hand.IsGrabbing() && !hand.holdingObj.maintainGrabOffset)
                {
                    ...
                    hand.grabPositionOffset = Vector3.MoveTowards(
                        hand.grabPositionOffset,
                        Vector3.zero,
                        (deltaDist) * hand.gentleGrabSpeed * Time.deltaTime * 60f);
                    hand.grabRotationOffset = Quaternion.RotateTowards(
                        hand.grabRotationOffset,
                        Quaternion.identity,
                        (deltaRot) * hand.gentleGrabSpeed * Time.deltaTime * 60f);
                ...
                void UpdateOffset(bool isGrab)
                {
                    ...
                    hand.grabPositionOffset = Vector3.MoveTowards(
                        hand.grabPositionOffset,
                        Vector3.zero,
                        grabReturnRotationDistance * smoothTime * Time.deltaTime);
                    hand.grabRotationOffset = Quaternion.RotateTowards(
                        hand.grabRotationOffset,
                        Quaternion.identity,
                        grabReturnPositionDistance * smoothTime * Time.deltaTime);
            ...
            else if (hand.enableMovement && hand.forwardFollow)
            {
                hand.transform.position = follow.position;
                hand.transform.rotation = follow.rotation;
            }
        ...
        internal virtual void MoveTo(float deltaTime)
        {
            ...
            {
                foreach (var handing in currentHands)
                    if (handing != null && handing != hand)
                        handing.handFollow.MoveTo(deltaTime);
            }

            float velocityChange = minVelocityChange;
            ...
            velocityChange *= deltaOffset;
            velocityChange *= 1 + (distance) * minVelocityDistanceMulti;
            ...
            if (currentHands != null)
            {
                towardsVel = new Vector3(
                    Mathf.MoveTowards(currentVelocity.x, vel.x, velocityChange),
                    Mathf.MoveTowards(currentVelocity.y, vel.y, velocityChange),
                    Mathf.MoveTowards(currentVelocity.z, vel.z, velocityChange));
            }
            else
            {
                towardsVel = new Vector3(
                    Mathf.MoveTowards(currentVelocity.x, vel.x, velocityChange / 5f + Mathf.Abs(currentVelocity.x) / 1.5f),
                    Mathf.MoveTowards(currentVelocity.y, vel.y, velocityChange / 5f + Mathf.Abs(currentVelocity.y) / 1.5f),
                    Mathf.MoveTowards(currentVelocity.z, vel.z, velocityChange / 5f + Mathf.Abs(currentVelocity.z) / 1.5f));
            }
        ...
        internal virtual void TorqueTo(float deltaTime)
        {
            ...
            if (currentHands != null)
            {
                foreach (var handing in currentHands)
                    if (handing != null && handing != hand)
                        handing.handFollow.TorqueTo(deltaTime);
            }
            ...
            if (currentHands == null)
                hand.body.angularDamping = Mathf.Lerp((startAngularDrag * angleDragDamper), startAngularDrag, angle / angleDragDamperDistance)
                                           * inverseDeltaOffset;
            else
        ...
        protected virtual void SetMass()
        {
            ...
            {
                foreach (var handing in currentHands)
                    if (handing != null && handing != this.hand)
                        handing.handFollow.SetMass();
            }
            ...
            var lerpPoint = Vector3.Distance(moveTo.position, currentPos) / distanceMassMaxDistance;
            var angleLerpPoint = Mathf.Abs(Quaternion.Angle(moveTo.rotation, currentRot)) / angleMassMaxAngle;
            ...
            if (hand.holdingObj != null && !hand.IsGrabbing())
            {
                ...
                var heldDistanceMass = Mathf.Lerp(startHeldMass * (minMass / maxMass), startHeldMass, lerpPoint)
                                       * distanceMassDifference
                                       / (distanceMassDifference + angleMassDifference);
                float heldAngleMass = Mathf.Lerp(startHeldMass * (minMass / maxMass), startHeldMass, angleLerpPoint)
                                      * angleMassDifference
                                      / (distanceMassDifference + angleMassDifference);
        ...
        protected virtual void AverageMass()
        {
            ...
            foreach (var handComp in currentHands)
            {
                averageMass += handComp.handFollow.targetMass;
                averageHeldMass += handComp.handFollow.targetHeldMass;
            }

            averageMass /= currentHands.Count;
            foreach (var handing in currentHands)
                handing.body.mass = averageMass;
        ...
        public virtual void SetHandLocation(Vector3 targetPosition, Quaternion targetRotation)
        {
            ...
            if (hand.holdingObj && hand.holdingObj.parentOnGrab)
            {
                if (!hand.IsGrabbing())
                {
                    ...
                    // grabRuler 是 handRuler 的子级，因此会继承其变换
                    ...
                    // 这里我猜是遗留代码未清除，因为 ignoreMoveFrame = true 已经阻止了物理更新（会清零）
        ...
        public virtual void SetMoveTo(bool ignoreRedundancyCheck = false)
        {
            ...
            if (ignoreRedundancyCheck
                || Time.fixedTime - lastSetMoveToTime > 0
            ...
            {
                foreach (var handing in currentHands)
                    if (handing != null && !handing.Equals(this.hand))
                        handing.handFollow.SetMoveTo();
            }

            var targetMoveToPos = follow.position + hand.grabPositionOffset;
            var targetMoveToRot = follow.rotation * hand.grabRotationOffset;

            if (hand.holdingObj != null)
            {
                if (hand.left)
                {
                    var moveLeft = hand.holdingObj.heldPositionOffset;
                    moveLeft.x *= -1;
                    var leftRot = -hand.holdingObj.heldRotationOffset;
                    leftRot.x *= -1;
                    targetMoveToPos += transform.rotation * moveLeft;
                    targetMoveToRot *= Quaternion.Euler(leftRot);
                }
                else
                {
                    targetMoveToPos += transform.rotation * hand.holdingObj.heldPositionOffset;
                    targetMoveToRot *= Quaternion.Euler(hand.holdingObj.heldRotationOffset);
                }
            }
            ...
            if (hand.holdingObj != null && !hand.IsGrabbing())
            {
                var distance = Vector3.Distance(targetMoveToPos, hand.handGrabPoint.position);
                var angleDistance = Quaternion.Angle(targetMoveToRot, hand.handGrabPoint.rotation);
                moveTo.position = Vector3.MoveTowards(
                    hand.handGrabPoint.position,
                    targetMoveToPos,
                    maxMoveToDistance + (Mathf.Sqrt(distance + 1f)) - 1f);
                moveTo.rotation = Quaternion.RotateTowards(
                    hand.handGrabPoint.rotation,
                    targetMoveToRot,
                    maxMoveToAngle + Mathf.Sqrt(angleDistance + 1f) - 1f);
            }
            else
            {
                var distance = Vector3.Distance(targetMoveToPos, hand.transform.position);
                var angleDistance = Quaternion.Angle(targetMoveToRot, hand.transform.rotation);
                moveTo.position = Vector3.MoveTowards(hand.transform.position, targetMoveToPos, maxMoveToDistance + (Mathf.Sqrt(distance + 1f) - 1f));
                moveTo.rotation = Quaternion.RotateTowards(
            ...
            targetMoveToPosition = targetMoveToPos;
            targetMoveToRotation = targetMoveToRot;
        ...
        public virtual void AverageSetMoveTo(bool ignoreRedundencyCheck = false)
        {
            ...
            foreach (var handing in currentHands)
            {
                if (handing != null)
                {
                    totalDistance += Vector3.Distance(handing.handFollow.targetMoveToPosition, handing.handGrabPoint.position);
                    totalAngleDistance += Quaternion.Angle(handing.handFollow.targetMoveToRotation, handing.handGrabPoint.rotation);
                }
            }
            ...
            if (currentHands != null)
            {
                foreach (var handing in currentHands)
                    if (handing != null && !handing.Equals(this.hand))
                        handing.handFollow.AverageSetMoveTo();
            }
            ...
            for (int i = 0; i < heldBy.Count; i++)
            {
                if (!heldBy[i].Equals(hand))
                {
                    ...
                    moveTo.rotation = Quaternion.Lerp(
                        moveTo.rotation,
                        moveTo.rotation * deltaRotation,
                        otherAngleDistance / totalAngleDistance * massOffset * 0.5f * totalDistanceDiff);
                }
            }
