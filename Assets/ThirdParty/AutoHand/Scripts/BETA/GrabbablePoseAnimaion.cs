...
namespace Autohand
{
    ...
    public class GrabbablePoseAnimaion : MonoBehaviour
    {
        ...
        [ShowIf("IsAnimationDriverCustom")]
        ...
        [Tooltip(
            "Additional animations to run alongside the given driver value (good for things like a gun trigger that is separate from the hand but still needs to move with the hand during the animation)")]
        public AutoAnimation[] additionalAnimations;
        [Space]
        [Tooltip(
            "The weight of the index finger in the animation - 0 means the finger will not animate, 1 means it will animate fully")]
        public float indexWeight = 1;
        [Tooltip(
            "The weight of the middle finger in the animation - 0 means the finger will not animate, 1 means it will animate fully")]
        public float middleWeight = 1;
        [Tooltip(
            "The weight of the ring finger in the animation - 0 means the finger will not animate, 1 means it will animate fully")]
        public float ringWeight = 1;
        [Tooltip(
            "The weight of the pinky finger in the animation - 0 means the finger will not animate, 1 means it will animate fully")]
        public float pinkyWeight = 1;
        [Tooltip(
            "The weight of the thumb finger in the animation - 0 means the finger will not animate, 1 means it will animate fully")]
        ...
        public ref HandPoseData currentAnimationPose
        {
            get { return ref _currentAnimationPose; }
        }
        ...
        public void Animate(Hand hand)
        {
            ...
            foreach (var finger in hand.fingers)
            {
                ...
                currentAnimationPose.fingerPoses[fingerIndex].LerpData(
                    ref fromPoseData.fingerPoses[fingerIndex],
                    ref toPoseData.fingerPoses[fingerIndex],
                    fingerWeights[fingerIndex] * animationValue);
                currentAnimationPose.fingerPoses[fingerIndex].SetFingerPose(finger);
            }

            if (handPositionWeight != 0 || handRotationWeight != 0)
            {
                currentAnimationPose.handOffset = Vector3.Lerp(
                    fromPoseData.handOffset,
                    toPoseData.handOffset,
                    animationValue * handPositionWeight);
                currentAnimationPose.localQuaternionOffset = Quaternion.Lerp(
                    fromPoseData.localQuaternionOffset,
                    toPoseData.localQuaternionOffset,
                    animationValue * handRotationWeight);
        ...
        public void Animate(Hand hand, float value)
        {
            HandPoseData.LerpPose(
                ref currentAnimationPose,
                ref fromPose.GetHandPoseData(hand),
                ref toPose.GetHandPoseData(hand),
                value);
        ...
        private bool IsAnimationDriverCustom => animationDriver == HeldAnimationDriver.custom;
    }
}