    ...
    public static class GameUtils
    {
        ...
        public static Type GetType(string typeFullName, string assemblyName = null)
        {
            ...
            foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
            {
                ...
                if (!string.IsNullOrEmpty(assemblyName) && currentAssemblyName != assemblyName) continue;

                foreach (var t in assembly.GetTypes())
                {
                    if (t.FullName == typeFullName) return t;
                }
            }
        ...
        {
            if (string.IsNullOrEmpty(path) || string.IsNullOrEmpty(extension)) return path;
        ...
        public static string ReadAllTextSafe(string path, Encoding encoding)
        {
            ...
            try
            {
                ...
                {
                    var result = GameUtils.WebGetBufferAsync(path).GetAwaiter().GetResult();
        ...
        public static async UniTask<string> ReadAllTextSafeAsync(
            string path,
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                if (Application.platform == RuntimePlatform.Android)
                {
                    var result = await GameUtils.WebGetBufferAsync(path);
        ...
        {
            if (!CreateFileDirectoryIfNotExist(path)) return;
        ...
        private static readonly byte[] _encryptionKey = Encoding.UTF8.GetBytes("Framework@202512*QWERTYU-Mnbvcxz#");
        ...
        public static string Encrypt(string text)
        {
            ...
            RijndaelManaged rm = new() { Key = _encryptionKey, Mode = CipherMode.ECB, Padding = PaddingMode.PKCS7 };
        ...
        private static async UniTask<WebRequestResult> WebRequestInternal(
            UnityWebRequest www,
            float timeout,
