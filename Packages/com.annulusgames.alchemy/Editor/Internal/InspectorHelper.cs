...
namespace Alchemy.Editor
{
    internal static class InspectorHelper
    {
        public sealed class GroupNode
        {
            ...
            readonly List<object> orderedItems = new();
            ...
            public IReadOnlyList<object> OrderedItems => orderedItems;
        ...
        public static void BuildElements(SerializedObject serializedObject, VisualElement rootElement, object target, Func<string, SerializedProperty> findPropertyFunc)
        {
            ...
            {
                foreach (var entry in node.OrderedItems)
                {
                    switch (entry)
                    {
                        case MemberInfo memberInfo:
                            AddMemberElement(node, memberInfo);
                            break;
                        case GroupNode childNode:
            ...
            void AttachChild(GroupNode parent, GroupNode child)
            {
                ...
                parent.VisualElement.Add(child.VisualElement);
            }

            void AddMemberElement(GroupNode node, MemberInfo member)
            {
                // Exclude if member has HideInInspector attribute
                if (member.HasCustomAttribute<HideInInspector>()) return;
