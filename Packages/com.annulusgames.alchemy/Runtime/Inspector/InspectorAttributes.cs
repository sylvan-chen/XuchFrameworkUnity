...
namespace Alchemy.Inspector
{
    ...
    public sealed class HideIfAttribute : Attribute
    {
        ...
        {
            Conditions = ConditionalAttributeUtility.ValidateConditions(conditions);
        }

        [Obsolete("Condition is deprecated, use Conditions instead.")]
        public string Condition => Conditions.Length > 0 ? Conditions[0] : null;
    ...
    static class ConditionalAttributeUtility
    {
        public static string[] ValidateConditions(string[] conditions)
        {
            ...
            if (conditions.Any(string.IsNullOrWhiteSpace))
            {
                throw new ArgumentException("Conditions cannot contain null or whitespace entries.", nameof(conditions));
            }
