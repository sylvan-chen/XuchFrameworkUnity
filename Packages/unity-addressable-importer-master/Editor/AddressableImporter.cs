...
public class AddressableImporter : AssetPostprocessor
{
    ...
    static void RemoveGroup(AddressableAssetSettings settings, string groupName)
    {
        ...
        AddressableImportSettings importSettings)
    {
        ...
        if (TryGetMatchedRule(assetPath, importSettings, out var matchedRule))
        {
            ...
            //遍历directoryName，找出各级路径是否在matchedRule.blackListNames中
            string[] directoryNames = directoryName.Split('/');
            string tempDirectory = "";
            bool blackDir = false;
            foreach (string directoryName2 in directoryNames)
            {
                tempDirectory += directoryName2;
                if (matchedRule.blackListNames.Contains(tempDirectory))
                {
                    blackDir = true;
                    break;
                }
            ...
            if (matchedRule.blackListNames.Contains(fileName) || blackDir)
            {
                Debug.LogFormat("[AddressableImporter] Entry removed for {0} because it is blacklisted", assetPath);
        ...
        string assetPath)
    {
        ...
        string togethorGroupName = rule.ParseGroupReplacement(assetPath);
        string fileName = Path.GetFileNameWithoutExtension(assetPath);
        fileName = fileName.Replace(".", "_");
        string fileGroupName = $"{togethorGroupName}_{fileName}";
        if (!rule.groupTogethor)
        {
            if(TryGetGroup(settings, togethorGroupName, out group))
            {
                RemoveGroup(settings, togethorGroupName);
            }
        }
        else
        {
            if(TryGetGroup(settings, fileGroupName, out group))
            {
                RemoveGroup(settings, fileGroupName);
            }
        }

        string groupName = togethorGroupName;
        if (!rule.groupTogethor)
        {
        ...
        // set Build&Load path as remote
        var profileId = settings.activeProfileId;
        var buildPath = AddressableAssetSettings.kLocalBuildPath;
        var loadPath = AddressableAssetSettings.kLocalLoadPath;
        if (!rule.isLocal)
        {
        ...
        if (group != null)
        {
            // Debug.Log($"[AddressableImporter] Group {group.Name} found.");
            var schema = group.GetSchema<BundledAssetGroupSchema>();
            if (schema != null)
            {
                // Debug.Log($"[AddressableImporter] Schema found.");
                schema.BuildPath.SetVariableById(settings, settings.profileSettings.GetProfileDataByName(buildPath).Id);
                schema.LoadPath.SetVariableById(settings, settings.profileSettings.GetProfileDataByName(loadPath).Id);
            }
        }
